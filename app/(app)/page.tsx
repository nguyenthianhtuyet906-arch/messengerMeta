...
import { getConversations, getCurrentUser } from "@/lib/messages"
...
export default async function DashboardPage() {
  ...
  const withMessages = conversations.filter((c) => c.lastMessage !== "Hãy bắt đầu trò chuyện").length

  const stats = [
    { label: "Tin nhắn chưa đọc", value: totalUnread, icon: MessageCircle },
    { label: "Cuộc trò chuyện", value: withMessages, icon: Users },
    { label: "Người dùng khác", value: conversations.length, icon: Clock },
  ]
    ...
    <div className="h-full overflow-y-auto bg-background">
      <div className="mx-auto max-w-4xl px-6 py-8 md:px-10 md:py-10">
        <header className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-balance">{`Chào mừng trở lại, ${firstName}`}</h1>
          ...
          <div className="overflow-hidden rounded-3xl border border-border bg-card">
            {conversations.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-muted-foreground">
                Chưa có người dùng nào khác. Hãy mời bạn bè đăng ký để bắt đầu trò chuyện.
              </p>
            ) : (
