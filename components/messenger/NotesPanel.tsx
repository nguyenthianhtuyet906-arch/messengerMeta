...
function NoteCard({
  ...
  return (
    <div className="rounded-2xl border border-[#dee3e9] p-3">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-xs font-bold text-[#0a1317]">{note.authorName}</p>
        ...
        {note.mine && !editing ? (
          <div className="flex shrink-0 items-center gap-1">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full text-[#5d6c7b] transition-colors hover:bg-[#f1f4f7]"
            >
            ...
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full text-[#5d6c7b] transition-colors hover:bg-[#fdeaec] hover:text-[#c41e3a]"
            >
      ...
      {editing ? (
        <div className="mt-2 flex items-end gap-2">
          <textarea
            ...
            rows={2}
            autoFocus
            className="max-h-32 flex-1 resize-none rounded-xl border-0 bg-[#f1f4f7] px-3 py-2 text-sm text-[#0a1317] focus:outline-none focus:ring-2 focus:ring-[#1876f2]"
          />
          <button
            onClick={save}
            aria-label="Lưu"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#0064e0] text-white transition-colors hover:bg-[#0457cb]"
          >
            <Check className="h-4 w-4" />
          </button>
        </div>
      ) : (
        <p className="mt-2 whitespace-pre-wrap break-words text-sm text-[#0a1317]">
          {note.body}
        </p>
      )}
    </div>
  ...
  return (
    <aside className="flex h-full w-full flex-col border-l border-[#dee3e9] bg-white">
      <header className="flex shrink-0 items-center justify-between border-b border-[#dee3e9] px-4 py-3">
        <h3 className="font-bold text-[#0a1317]">Ghi chú</h3>
        {onClose ? (
          <button
            onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {isLoading ? (
          ...
          <p className="px-1 py-4 text-sm text-[#c41e3a]">Không tải được ghi chú.</p>
        ) : notes.length === 0 ? (
          <p className="px-1 py-4 text-sm text-[#5d6c7b]">Chưa có ghi chú nào.</p>
        ) : (
      ...
      <div className="flex shrink-0 items-end gap-2 border-t border-[#dee3e9] px-4 py-4">
        <textarea
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          onKeyDown={(e: KeyboardEvent<HTMLTextAreaElement>) => {
