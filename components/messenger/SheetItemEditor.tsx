...
function CopyCode({ value, className }: { value: string; className?: string }) {
  ...
  return (
    <button
      ...
      title="Bấm để copy"
      className={"min-w-0 select-text text-left hover:text-[#0064e0] " + (className ?? "")}
    >
      {value}
      {copied ? <span className="ml-1 text-xs text-[#1a7f37]">đã copy</span> : null}
    </button>
  );
}
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between gap-2 rounded-lg border-0 bg-[#f1f4f7] px-2.5 py-1.5 text-left text-sm focus:outline-none focus:ring-2 focus:ring-[#1876f2] disabled:opacity-60"
      >
        <span className={value ? "truncate" : "truncate text-[#9aa6b2]"}>
          {value || "— Chọn trạng thái —"}
        </span>
        <ChevronDown className="h-4 w-4 shrink-0 text-[#5d6c7b]" />
      </button>

      {open ? (
        <div className="absolute z-30 mt-1 w-full overflow-hidden rounded-lg border border-[#dee3e9] bg-white shadow-lg">
          <div className="border-b border-[#eef1f4] p-1.5">
            <input
              ...
              className="w-full rounded-md bg-[#f1f4f7] px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-[#1876f2]"
            />
          </div>
          <div className="max-h-56 overflow-y-auto py-1">
            <button
              type="button"
              onClick={() => pick("")}
              className="block w-full px-3 py-1.5 text-left text-sm text-[#9aa6b2] hover:bg-[#f1f4f7]"
            >
              — Chọn trạng thái —
            </button>
            {filtered.map((o) => (
              <button
                ...
                onClick={() => pick(o)}
                className={
                  "block w-full px-3 py-1.5 text-left text-sm hover:bg-[#f1f4f7] " +
                  (o === value ? "bg-[#e7f0fb] font-semibold text-[#0064e0]" : "text-[#0a1317]")
                }
              >
                {o}
              </button>
            ...
            {filtered.length === 0 ? (
              <p className="px-3 py-2 text-xs text-[#9aa6b2]">Không có kết quả</p>
            ) : null}
          </div>
        </div>
      ) : null}
...
function FieldInput({
  ...
  return (
    <input
      ...
      disabled={disabled}
      onChange={(e) => onChange(e.target.value)}
      className="w-full rounded-lg border-0 bg-[#f1f4f7] px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1876f2] disabled:opacity-60"
    />
  );
}
...
function SheetRowDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-[#dee3e9] px-4 py-3">
          <div className="min-w-0">
            <h3 className="truncate font-bold text-[#0a1317]">{match.itemId}</h3>
            <p className="truncate text-xs text-[#5d6c7b]">
          ...
          <button
            onClick={onClose}
            aria-label="Đóng"
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[#5d6c7b] hover:bg-[#f1f4f7]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {headers.map((h) => (
              <label key={h} className="block">
                <span className="mb-1 block text-xs font-semibold text-[#0a1317]">
                  {h}
                  {READONLY_FIELDS.has(h) ? (
                    <span className="ml-1 font-normal text-[#9aa6b2]">(khoá)</span>
                  ) : null}
                </span>
                <FieldInput
                  field={h}
        ...
        <div className="flex shrink-0 items-center justify-end gap-2 border-t border-[#dee3e9] px-4 py-3">
          <button
            onClick={onClose}
            className="rounded-full bg-[#f1f4f7] px-4 py-2 text-sm font-medium text-[#5d6c7b] hover:bg-[#e4e8ec]"
          >
            Đóng
          </button>
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-[#0064e0] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0457cb] disabled:bg-[#bcc0c4]"
          >
            {updateRow.isPending ? (
...
function MatchEditor({ match, onSaved }: { match: OrderRowMatch; onSaved: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-[#dee3e9] bg-white p-2.5">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="shrink-0 text-[#5d6c7b] hover:text-[#0a1317]"
        >
          <ChevronDown className={"h-3.5 w-3.5 transition-transform " + (open ? "" : "-rotate-90")} />
        </button>
        <CopyCode value={match.itemId} className="flex-1 break-all text-xs font-bold text-[#0a1317]" />
        {!open && dirtyFields.length > 0 ? (
          <span className="shrink-0 text-[11px] text-[#b54708]">• chưa lưu</span>
        ) : null}
        <button
        ...
        <>
          <div className="mt-2 flex flex-col gap-2.5">
            {fields.map((f) => (
              <label key={f} className="block">
                <span className="mb-1 block text-xs font-semibold text-[#0a1317]">{f}</span>
                <FieldInput
                  field={f}
                  value={draft[f] ?? ""}
          ...
          <div className="mt-2.5 flex items-center gap-2">
            ...
            {dirtyFields.length > 0 ? (
              <span className="text-xs text-[#5d6c7b]">{dirtyFields.length} thay đổi chưa lưu</span>
            ) : null}
          </div>
        </>
      ) : null}
...
export function SheetReceiptEditor({
  ...
  return (
    <div className="rounded-2xl border border-[#dee3e9] bg-white">
      <div className="flex items-center gap-2 p-2.5">
        ...
        <CopyCode value={orderCode} className="flex-1 break-all text-xs font-bold text-[#0a1317]" />
        {open && matches.length > 1 ? (
          <span className="shrink-0 text-[11px] text-[#9aa6b2]">{matches.length} dòng</span>
        ) : null}
        <button
          onClick={refresh}
          disabled={busy}
          ...
          className="shrink-0 text-[#5d6c7b] hover:text-[#0064e0] disabled:opacity-50"
        >
      ...
      {open ? (
        <div className="border-t border-[#eef1f4] p-2.5">
          {resolve.isLoading ? (
            <p className="text-xs text-[#5d6c7b]">Đang tìm trong sheet…</p>
          ) : resolve.isError ? (
            (() => {
              ...
              if (e instanceof ApiError && e.code === "google_not_connected") {
                return (
                  <p className="text-xs text-[#b54708]">
                    Chưa kết nối Google.{" "}
                    <Link href="/settings" className="font-semibold text-[#0064e0] underline">
                      Kết nối
                    </Link>
                  </p>
                );
              }
              return <p className="text-xs text-[#b42318]">Lỗi tra cứu sheet.</p>;
            })()
          ) : matches.length === 0 ? (
